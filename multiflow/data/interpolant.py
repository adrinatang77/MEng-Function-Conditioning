...
from . import so3_utils, all_atom
...
class Interpolant:
    ...
    def _trans_euler_step(self, d_t, t, trans_1, trans_t, trans_1_uncond=None):
        assert d_t >= 0
        trans_vf = self._trans_vector_field(t, trans_1, trans_t)
        return trans_t + trans_vf * d_t

    def _rots_euler_step(self, d_t, t, rotmats_1, rotmats_t, rotmats_1_uncond=None):
        ...
        return so3_utils.geodesic_t(scaling * d_t, rotmats_1, rotmats_t, mat_uncond=rotmats_1_uncond, guidance_weight=1.0)
    ...
    def _aatypes_euler_step(self, d_t, t, logits_1, aatypes_t, logits_1_uncond=None):
        # S = 21
        ...
        if self._aatypes_cfg.interpolant_type == "masking":
            ...
            out = torch.multinomial(step_probs.view(-1, S), num_samples=1).view(batch_size, num_res)
            # out[1:] = out[0] ############# SPECIAL FOR DETERMINISTIC SAMPLING ################################
            return out
        elif self._aatypes_cfg.interpolant_type == "uniform":
            assert S == 20
    ...
    def _aatypes_euler_step_purity(self, d_t, t, logits_1, aatypes_t, logits_1_uncond=None):
        batch_size, num_res, S = logits_1.shape
        assert aatypes_t.shape == (batch_size, num_res)
        assert S == 21
        ...
        number_to_unmask = torch.binomial(count=torch.count_nonzero(aatypes_t == du.MASK_TOKEN_INDEX, dim=-1).float(),
                                          prob=unmask_probs)
        # number_to_unmask[1:] = number_to_unmask[0]   ############# SPECIAL FOR DETERMINISTIC SAMPLING ################################
        unmasked_samples = torch.multinomial(pt_x1_probs.view(-1, S-1), num_samples=1).view(batch_size, num_res)
        # unmasked_samples[1:] = unmasked_samples[0]   ############# SPECIAL FOR DETERMINISTIC SAMPLING ################################
    ...
    def sample(
        ...
        batch = {
            ...
            'trans_sc_unguided': trans_sc.clone(),
            'aatypes_sc_unguided': aatypes_sc.clone(),
        ...
        for t_2 in tqdm.tqdm(ts[1:]):
            ...
            with torch.no_grad():
                model_out = model(copy.deepcopy(batch))
                pred_trans_1_uncond = None
                pred_rotmats_1_uncond = None
                pred_logits_1_uncond = None
            ...
            if self._cfg.self_condition:
                ...
                batch['aatypes_sc'] = _trans_diffuse_mask(pred_logits_1, logits_1, diffuse_mask)
             # Take reverse step            
            trans_t_2 = self._trans_euler_step(
                d_t, t_1, pred_trans_1, trans_t_1, pred_trans_1_uncond)
            rotmats_t_2 = self._rots_euler_step(
                d_t, t_1, pred_rotmats_1, rotmats_t_1, pred_rotmats_1_uncond)
            if self._aatypes_cfg.do_purity:
                aatypes_t_2 = self._aatypes_euler_step_purity(d_t, t_1, pred_logits_1, aatypes_t_1, pred_logits_1_uncond)
                # aatypes_t_2 = aatypes_t_1 #################################### HIGHLY TEMPORARY ##########################################
            else:
                aatypes_t_2 = self._aatypes_euler_step(d_t, t_1, pred_logits_1, aatypes_t_1, pred_logits_1_uncond)
