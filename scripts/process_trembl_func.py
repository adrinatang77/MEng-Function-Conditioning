...
for filename in files:
    ...
    try:
        with open(dir + filename, 'r') as f:
            go_terms_and_ec_numbers = {}
            protein_entries = ast.literal_eval(json.dumps(f)) # json.load(f)
            for line in protein_entries:
                # print('line', line)
                # pbar.update()
                total_proteins += 1
            ...
            with open(args.out + filename, 'w') as json_file:
                json.dump(go_terms_and_ec_numbers, json_file, indent=4)
                # print('Completed file.')
    except:
        print('Error in file ' + str(filename), flush=True)
    
    print('Done file ' + str(file_ind), flush=True)
